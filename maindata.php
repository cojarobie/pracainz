  ...
  try {
    ...
    if ($connection->connect_errno != 0) {
      ...
      if ($result = $connection->query("SELECT t.Name AS t_name, tu.ID as tu_id FROM users AS u INNER JOIN teams_users AS tu ON tu.id_users=u.id INNER JOIN teams AS t ON t.id=tu.id_teams WHERE u.id=$id AND tu.ustatus='member'")) {
         while ($row = $result->fetch_assoc()) {
           $team = to_array($row['t_name'], 'Member',  '<div id="leave'.$row['tu_id'].'" ><button type="button" class="btn btn-danger leave" onclick="leave('. $row['tu_id'] .')">Leave<i class="icon-logout"></i></button></div>');
      ...
      if ($result = $connection->query("SELECT l.Name AS l_name FROM teams AS t LEFT JOIN teams_users AS tu ON t.id = tu.id_teams INNER JOIN teams_leagues AS tl ON t.id = tl.id_Team INNER JOIN leagues AS l ON l.id = tl.id_league WHERE t.id_captain=$id OR ( tu.id_users=$id AND tu.Ustatus='memeber');")) {
         while ($row = $result->fetch_assoc()) {
